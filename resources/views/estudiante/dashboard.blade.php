    ...
    <div class="row">
                ...
                <nav class="nav flex-column px-3 mt-2">
                    ...
                    @if(in_array($rolNombre, [
                        ...
                        @if($rolNombre === 'AdministradorSistema')
                            {{-- AdminSistema: menú especial de estudiantes --}}
                        ...
                        @elseif($rolNombre === 'Docente')
                            {{-- Docente: Consultar estudiantes -> notas, materia, asistencias --}}
                            <a class="nav-link text-dark d-flex justify-content-between align-items-center"
                               data-bs-toggle="collapse"
                            ...
                            <div class="collapse ps-4" id="menuDocenteEstudiantes">
                                <a class="nav-link text-dark" href="{{ route('docente.registrar_notas') }}">
                                    <i class="fas fa-clipboard-list me-2"></i>Consultar / registrar notas
                                </a>
                                <a class="nav-link text-dark" href="#">
                    ...
                    {{-- CITAS DE ORIENTACIÓN (según rol) --}}
                    ...
                    {{-- ESTUDIANTE: MATERIAS / NOTAS / BOLETINES --}}
                    ...
                    @if(in_array($rolNombre, ['Estudiante', 'Acudiente']))
                        <a class="nav-link text-dark"
                           href="{{ $rolNombre === 'Estudiante' ? route('estudiante.reportes_disciplinarios') : '#' }}">
                    ...
                    @if(in_array($rolNombre, ['Estudiante', 'Acudiente', 'Docente', 'Orientador']))
                        <a class="nav-link text-dark"
                           href="{{ $rolNombre === 'Estudiante' ? route('estudiante.notificaciones') : '#' }}">
                            <i class="fas fa-bell me-2"></i>Centro de notificaciones
                        </a>
                    @endif

                    {{-- INFORMACIÓN DEL COLEGIO / INSTITUCIONAL --}}
        ...
        <div class="col-md-9 col-lg-10">
            <div class="main-content p-4">
                ...
                @if($rolNombre === 'Orientador')
                    @php
                        $pendientes = \App\Models\Cita::where('orientador_id', auth()->id())
                                        ->where('estado', 'pendiente')
                                        ->count();
                        $programadas = \App\Models\Cita::where('orientador_id', auth()->id())
                                        ->where('estado', 'programado')
                                        ->count();
                        $realizadas = \App\Models\Cita::where('orientador_id', auth()->id())
                                        ->where('estado', 'realizado')
                                        ->count();
                        $estudiantesSeguimiento = \App\Models\Cita::where('orientador_id', auth()->id())
                        ...
                        <div class="card border-0 shadow-sm">
                            ...
                            <div class="card-body">
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted mb-1">No hay actividades recientes para mostrar.</p>
                                    <p class="text-muted mb-0">¡Comienza registrando estudiantes o docentes!</p>
                                </div>
                            </div>
                        </div>
                    </div>
